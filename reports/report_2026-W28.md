# Israeli DevOps Job Market Report — 2026-W28

_Generated July 09, 2026 • Maintained by [Orel Bello](https://www.linkedin.com/in/orel-bello/) (FlipTheScript • AWS Community Builder)_


## Executive Summary

We tracked **94 open DevOps / SRE / Platform / Cloud roles** in Israel this week
across 2 sources. **2 of those (2.1%)
are junior-friendly** — a key metric for FlipTheScript's 600+ mentees.

## Week-over-Week Trends

📉 **Total jobs**: -5 (-5.1%) vs 2026-W27
📊 **Junior % change**: +2.1 pp

**Rising skills:**
- Python: 11 → 12 (+1)

**Falling skills:**
- CI/CD: 21 → 17 (-4)
- Kubernetes: 18 → 15 (-3)
- Linux: 12 → 10 (-2)
- Docker: 11 → 9 (-2)
- Security: 21 → 19 (-2)

**New companies in top 15:** Infinidat, Paragon, Mobileye, Gotfriends

## Top 10 In-Demand Skills

1. **Security** — 19 mentions
2. **AWS** — 18 mentions
3. **CI/CD** — 17 mentions
4. **Kubernetes** — 15 mentions
5. **Azure** — 13 mentions
6. **Python** — 12 mentions
7. **GCP** — 11 mentions
8. **Terraform** — 10 mentions
9. **Linux** — 10 mentions
10. **Observability** — 10 mentions

## Top 10 Hiring Companies

1. **Gongio** — 5 open roles
2. **Via** — 3 open roles
3. **NVIDIA** — 3 open roles
4. **Nice** — 2 open roles
5. **Payoneer** — 2 open roles
6. **Jfrog** — 2 open roles
7. **Taboola** — 2 open roles
8. **Similarweb** — 2 open roles
9. **Appsflyer** — 2 open roles
10. **Transmitsecurity** — 2 open roles

## Seniority Distribution

- **Junior**: 2.1% ``
- **Mid**: 55.3% `██████████████████████`
- **Senior**: 42.6% `█████████████████`

## Salary Insights (USD, where parseable)

- **Junior**: _insufficient data_ (n=0)
- **Mid**: _insufficient data_ (n=0)
- **Senior**: _insufficient data_ (n=0)

> _Salary data is best-effort: most Israeli postings don't disclose ranges publicly. Numbers are extracted only when explicitly stated._

## Location Breakdown

- **Tel Aviv** — 65
- **Israel (other)** — 18
- **Raanana** — 3
- **Petah Tikva** — 3
- **Herzliya** — 2
- **Haifa** — 2
- **Jerusalem** — 1

## Sources

- linkedin: 67
- greenhouse: 27

## Methodology

Data is collected weekly from public job-board endpoints (LinkedIn guest API, RemoteOK, Greenhouse boards
of major Israeli tech companies, AllJobs, Drushim, Glassdoor, Jobmaster). Each posting is filtered for
DevOps / SRE / Platform Engineering / Cloud roles in Israel, deduplicated across sources, and analyzed
for skills, seniority, salary, and location. Skill mentions are extracted via a curated regex taxonomy.

**100% free • Open methodology • No paid APIs.**

---

**Made for the Israeli DevOps community by [FlipTheScript](https://www.linkedin.com/groups/12877927/).**
